  ...
  return (
    <div className={cn("flex flex-col gap-1 mt-2.5", className)} role="status">
      <div className="flex items-center gap-3">
        <div className="flex flex-1 gap-1.5">
          {Array.from({ length: SEGMENT_COUNT }).map((_, index) => {
            ...
            return (
              <div
                key={index}
                className="h-1.5 flex-1 overflow-hidden rounded-full bg-[#0c1014]/10"
              >
                <motion.div
                  className="h-full w-full origin-left rounded-full"
                  initial={false}
        ...
        <span className="relative inline-grid h-4 shrink-0 items-center justify-items-end overflow-hidden font-body text-xs font-semibold">
          {Array.from(new Set(STRENGTH_LABELS)).map((spacer) => (
            <span
              key={spacer}
              aria-hidden="true"
          ...
          <AnimatePresence mode="popLayout" initial={false} custom={direction}>
            <motion.span
              key={strengthLabel}
              className={cn(
                "col-start-1 row-start-1 font-body text-xs font-semibold transition-colors",
                LABEL_CLASSES[score],
              )}
              custom={direction}
              variants={labelVariants}
