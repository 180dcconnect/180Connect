 ...
 * replies ("always-immediate for replies per F174"). The two strings are
 * F174's actual producer types (the F133 trigger on reply_events emits
 * `client_reply_received` for the client's owner and
 * `unowned_client_reply_received` for the admin fallback) — there is no
 * bare `reply_received` type anywhere on dev. The override is a general
 * rule, not a one-off special case: a future high-priority producer opts
 * in here once, rather than every read path having to know about it
 * separately.
 */
const ALWAYS_IMMEDIATE_TYPES: ReadonlySet<string> = new Set([
