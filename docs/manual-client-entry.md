...
`src/lib/manual-entry.ts` defines `ManualEntryApprovalChecks`. Approval must call
all three checks and fails closed unless every result is `passed`:
...
- **F047** — client-criteria result.

The admin Approve control is intentionally disabled until those adapters are
connected. Reject remains available because it cannot create an active client.
