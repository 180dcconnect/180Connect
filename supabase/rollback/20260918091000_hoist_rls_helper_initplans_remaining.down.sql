-- Rollback for: 20260918091000_hoist_rls_helper_initplans_remaining.sql
...
--
-- Generated from `pg_policies` alongside the forward migration, so these are the
-- exact expressions that were in place before it ran.
...
create policy actions_delete_own_open on public.actions
  for delete to authenticated
  using ((app.is_active_user() AND app.is_cam() AND COALESCE((created_by_user_id = auth.uid()), false) AND COALESCE((assignee_user_id = auth.uid()), false) AND (status = 'open'::action_status)));
...
create policy actions_insert_cam on public.actions
  for insert to authenticated
  with check ((app.is_active_user() AND app.is_cam() AND (created_by_user_id = auth.uid()) AND (assignee_user_id = auth.uid()) AND (app.owns_organisation(organisation_id) OR app.organisation_is_unowned(organisation_id))));
...
create policy actions_update_assignee on public.actions
  for update to authenticated
  using ((app.is_active_user() AND app.is_cam() AND COALESCE((assignee_user_id = auth.uid()), false)))
  with check ((app.is_active_user() AND app.is_cam() AND COALESCE((assignee_user_id = auth.uid()), false)));
...
create policy audit_log_select_client_timeline on public.audit_log
  for select to authenticated
  using ((app.is_active_user() AND (target_table = 'organisations'::text) AND (action = ANY (ARRAY['status_changed'::text, 'ownership_reassigned'::text]))));
...
create policy booklet_generations_insert on public.booklet_generations
  for insert to authenticated
  with check ((app.is_active_user() AND app.can_contact_organisation(organisation_id) AND (generated_by = ( SELECT auth.uid() AS uid))));
...
create policy edit_suggestions_select_visible on public.edit_suggestions
  for select to authenticated
  using ((app.is_active_user() AND (app.is_admin() OR (requested_by = ( SELECT auth.uid() AS uid)) OR ((status = 'pending'::edit_suggestion_status) AND app.is_cam()))));
...
create policy feedback_insert on public.feedback
  for insert to authenticated
  with check ((app.is_active_user() AND (user_id = ( SELECT auth.uid() AS uid))));
...
create policy feedback_select_own on public.feedback
  for select to authenticated
  using ((app.is_active_user() AND (user_id = ( SELECT auth.uid() AS uid))));
...
create policy manual_entry_records_select_own_or_admin on public.manual_entry_records
  for select to authenticated
  using ((app.is_active_user() AND ((submitted_by_user_id = ( SELECT auth.uid() AS uid)) OR app.is_admin())));
...
create policy notes_delete_own on public.notes
  for delete to authenticated
  using ((app.is_active_user() AND (app.is_admin() OR COALESCE((author_id = auth.uid()), false))));
...
create policy notes_update_own on public.notes
  for update to authenticated
  using ((app.is_active_user() AND (app.is_admin() OR COALESCE((author_id = auth.uid()), false))))
  with check ((app.is_active_user() AND (app.is_admin() OR COALESCE((author_id = auth.uid()), false))));
...
create policy notifications_select_own on public.notifications
  for select to authenticated
  using ((app.is_active_user() AND (recipient_user_id = ( SELECT auth.uid() AS uid))));
...
create policy notifications_update_own_read_state on public.notifications
  for update to authenticated
  using ((app.is_active_user() AND (recipient_user_id = ( SELECT auth.uid() AS uid))))
  with check ((app.is_active_user() AND (recipient_user_id = ( SELECT auth.uid() AS uid))));
...
create policy outreach_messages_delete_admin on public.outreach_messages
  for delete to authenticated
  using ((app.is_active_user() AND app.is_admin() AND (send_status = 'draft'::send_status)));
...
create policy outreach_messages_delete_own_draft on public.outreach_messages
  for delete to authenticated
  using ((app.is_active_user() AND app.is_cam() AND COALESCE((sent_by_user_id = auth.uid()), false) AND (send_status = 'draft'::send_status)));
...
create policy outreach_messages_insert_admin on public.outreach_messages
  for insert to authenticated
  with check ((app.is_active_user() AND app.is_admin() AND app.can_contact_organisation(organisation_id)));
...
create policy outreach_messages_insert_cam on public.outreach_messages
  for insert to authenticated
  with check ((app.is_active_user() AND app.is_cam() AND (sent_by_user_id = auth.uid()) AND app.can_contact_organisation(organisation_id)));
...
create policy outreach_messages_update_own_draft on public.outreach_messages
  for update to authenticated
  using ((app.is_active_user() AND app.is_cam() AND COALESCE((sent_by_user_id = auth.uid()), false) AND (send_status = 'draft'::send_status)))
  with check ((app.is_active_user() AND app.is_cam() AND COALESCE((sent_by_user_id = auth.uid()), false)));
...
create policy ownership_requests_select_involved on public.ownership_requests
  for select to authenticated
  using ((app.is_active_user() AND (app.is_admin() OR (requested_by = ( SELECT auth.uid() AS uid)) OR app.owns_organisation(organisation_id))));
...
create policy restricted_edit_fields_select_admin on public.restricted_edit_fields
  for select to authenticated
  using ((app.is_active_user() AND (app.is_admin() OR (app.is_cam() AND active))));
