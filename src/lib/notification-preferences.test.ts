...
describe("shouldDeliverImmediately (F178 AC2/AC3)", () => {
  ...
  it("overrides a daily/weekly preference for F174's reply types (AC3)", () => {
    // client_reply_received = the owner's reply notification (F174/F133 trigger).
    assert.equal(shouldDeliverImmediately("daily", "client_reply_received"), true);
    assert.equal(shouldDeliverImmediately("weekly", "client_reply_received"), true);
    // unowned_client_reply_received = the admin fallback for an unowned client.
    assert.equal(shouldDeliverImmediately("daily", "unowned_client_reply_received"), true);
    assert.equal(shouldDeliverImmediately("weekly", "unowned_client_reply_received"), true);
  });

  it("does not treat the phantom pre-F174 type as always-immediate", () => {
    // Nothing on dev emits a bare `reply_received` — only the two types above.
    assert.equal(shouldDeliverImmediately("daily", "reply_received"), false);
  });
});
