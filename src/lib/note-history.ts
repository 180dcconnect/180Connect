/**
 * F071/F072/F073/F074 — notes-section logic behind the client detail page,
...
/** A note as the page renders it, with whether the viewer may manage it (F073 AC1, F074 AC1). */
export type NoteListItem = DisplayNote & { canManage: boolean };
...
/**
 * Whether `actor` may edit or delete `note` (F073 AC1, F074 AC1): the note's
 * own author, or an admin — matching `notes_update_own` and `notes_delete_own`
 * exactly (20260804180000_create_org_children.sql, both share this identical
 * predicate), so a button this drives is never shown for an action the API
 * routes would refuse anyway.
 */
...
/** Composes the two above into what the page actually needs, in order. */
export function buildNoteList(
  rows: readonly NoteRow[],
  actor: { id: string; role: string },
): NoteListItem[] {
  return orderNotesNewestFirst(rows).map((row) => ({
    ...buildDisplayNote(row),
    canManage: canManageNote(row, actor),
  }));
}
