/**
 * F071 — notes-section logic behind the client detail page, kept out of the
 * route so it can be tested without a database (same split as
...
/**
 * Orders notes newest-first (AC3) — a stable order rather than one that can
 * shuffle between visits, and it matches `notes_organisation_idx`'s own
...
/** Builds what the page actually renders for one note (AC2). */
