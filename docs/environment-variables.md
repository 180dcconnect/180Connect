...
| `NEWS_HOOK_PROVIDER` | `exa` | `exa` | Only server-side | Live news hook (F110): `exa` pulls one recent item, `none` disables the lookup so emails generate without a hook. Runs on every Stage 2 follow-up, and on a first-contact email whenever the CAM picks the news-hook opening. Optional, defaults to `none` |
| `EXA_API_KEY` | dev key | prod key (or same free key) | Only server-side | **SENSITIVE:** Exa key for the F110 news hook (both stages). Free tier at exa.ai needs no card ($10/month; blocked, never billed, on exhaustion). **Required when `NEWS_HOOK_PROVIDER` is `exa`** — startup fails otherwise |
