...
export function ScheduledEmailList({ organisationId, messages }: { organisationId: string; messages: { id: string; subject: string; scheduled_at: string }[] }) {
  ...
  return (
    <div className="mt-4 space-y-2">
      <h3 className="text-sm font-semibold text-ink">Scheduled</h3>
      {messages.map((message) => (
        <div className="flex flex-wrap items-center justify-between gap-2 rounded-inset border border-hold/25 bg-hold-wash p-3" key={message.id}>
          ...
          <OriginButton disabled={busy === message.id} onClick={async () => { setBusy(message.id); setNotice(null); const result = await cancelScheduledEmail({ organisationId, messageId: message.id }); setNotice({ ok: result.ok, message: result.message }); setBusy(null); }} size="sm" type="button" variant="outline">Cancel schedule</OriginButton>
        </div>
      ))}
      {notice && (
        <p
