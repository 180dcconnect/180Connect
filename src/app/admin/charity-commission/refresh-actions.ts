...
/**
 * The "Refresh register" button, which asks GitHub to rebuild the register file.
 *
 * The rebuild reads ~1.8GB of extracts and takes about twelve minutes, which no
 * serverless function will hold. So this does not do the work — it dispatches
 * the workflow that does, and reports back on it. From the screen it is one
 * button and a progress line; nobody opens a terminal, which is the whole point.
 *
 * ── What it needs ──
 *
 * `GITHUB_REGISTER_TOKEN`: a fine-grained token with **Actions: read and write**
 * on this repository only. Nothing else — it cannot read code, secrets or
 * anything outside Actions on one repo.
 */

const WORKFLOW = "refresh-charity-register.yml";
 ...
 * this screen uses: choosing which charities to import is everyday work, but
 * spending a CI run and redeploying the app is not, and a stuck refresh is the
 * kind of thing one person should own.
 */
export async function refreshRegister(): Promise<RefreshState> {
  ...
  try {
    ...
    return {
      kind: "started",
      message:
        "Refresh started. It takes about twelve minutes, then the app redeploys " +
        "with the new register. You can leave this page.",
    };
  } catch (error) {
    await reportError(error, {
      operation: "admin.charity_register.refresh",
 ...
 */
export async function refreshStatus(): Promise<RefreshState> {
  const authorization = await getCurrentActor("platform-settings:manage");
  if (!authorization.ok) {
    return { kind: "error", message: actorFailureMessage(authorization.reason) };
  }
