 ...
 * as two tabs under one sidebar entry.
 *
 ...
 * Both members are admin-only (`platform-settings:manage`), unlike Data
...
export function aiTabsFor(role: AppRole): GroupTabsSpec {
  return AI_TABS.filter((tab) => hasPermission(role, tab.permission));
}
