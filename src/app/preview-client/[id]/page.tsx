...
export default async function PreviewClientOverviewPage({
  params,
}: {
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1.55fr)_minmax(0,1fr)]">
            ...
            <SectionCard headingId="contactability-heading" title="Contactability">
              <dl className="mt-4 space-y-3">
                ...
                <div className="flex flex-wrap items-start gap-x-4 gap-y-2 rounded-xl border border-black/[0.05] bg-black/[0.015] px-4 py-3.5">
                  ...
                  <div className="min-w-[12rem] flex-1">
                    <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1.5">
                      <dt className="text-[11px] font-bold tracking-[0.12em] text-foreground/35 uppercase">Website</dt>
                      <Pill tone={website.status === "reachable" ? "go" : "stop"}>
                        {website.status === "reachable" ? "Reachable" : website.status === "invalid" ? "Invalid URL" : website.status === "missing" ? "Missing" : "Unreachable"}
                      </Pill>
                    </div>
                    <dd className="mt-1.5 text-sm leading-[1.6]">
                      {websiteLink ? (
                        <a
                          ...
                          className={`group inline-flex items-center gap-1.5 break-all underline decoration-1 underline-offset-2 transition-colors ${website.status === "reachable" ? "text-brand-hover hover:text-brand" : "font-bold text-destructive"}`}
                          href={websiteLink}
                          rel="noreferrer"
                          target="_blank"
                        ...
                        >
                          ...
                          <ExternalLink aria-hidden="true" className="h-3.5 w-3.5 shrink-0 opacity-60 transition-opacity group-hover:opacity-100 group-focus-visible:opacity-100" />
                        </a>
                      ) : website.url ? (
                        <span className="font-bold break-all text-destructive">{website.url}</span>
                      ) : (
                        <span className="text-foreground/35">Not provided</span>
                      )}
                    </dd>
                    {websiteLink && <p className="mt-1 text-[11px] font-medium tracking-wide text-foreground/40">Tap to open in new tab</p>}
                    {website.message && (
                      <p className="mt-1.5 text-[13px] leading-[1.6] text-destructive/80" role="alert">
        ...
        <Group className="space-y-6">
          ...
          <Rise>
            ...
            >
              {sourcesResult.error ? (
                <p className="mt-4 text-sm font-bold text-destructive" role="alert">Source information could not be loaded. Refresh and try again.</p>
              ) : sources.length === 0 ? (
                <p className="mt-4 text-sm leading-[1.7] text-foreground/45">No source information recorded.</p>
              ) : (
