...
--
--   claim_outreach_send(uuid)          (last redefined by 20260913100100 —
--                                       F128's atomic daily-cap body is kept
--                                       verbatim below; F018 adds the predicate)
...
--               | signature and snapshot behaviour verbatim. claim_outreach_send
--               | keeps F128's atomic daily-cap enforcement (20260913100100)
--               | verbatim — only its authorisation predicate changes. This file
--               | is therefore dated AFTER 20260913100100 so fresh replays and
--               | incremental applies converge on the same function body.
--               | Behaviour narrows
...
--
-- Reversibility: paired rollback in ../rollback/20260915000000_f018_contact_permission_rpc_checks.down.sql
...
comment on function public.claim_outreach_send(uuid) is
  'F123/F018/F128: atomically claim a draft for sending. Returns true once per '
  'unsent draft (false for everyone else until the claim goes stale or is '
  'released), refuses anyone but the client''s owner, an admin, or the draft''s '
  'author ON AN UNOWNED CLIENT with 42501, suppressed clients with P0001, and a '
  'reached branch-wide daily send cap with P0003 — all before any provider '
  'call. Not audited — the audited transition is mark_outreach_sent.';
