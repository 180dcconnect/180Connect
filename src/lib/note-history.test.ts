...
describe("canManageNote", () => {
  it("lets the author manage their own note", () => {
    assert.equal(
      canManageNote(note({ author_id: "cam-1" }), { id: "cam-1", role: "cam" }),
      true,
    );
  });

  it("refuses a different CAM (F073 AC1, F074 AC1)", () => {
    assert.equal(
      canManageNote(note({ author_id: "cam-1" }), { id: "cam-2", role: "cam" }),
      false,
    );
  });

  it("lets an admin manage any note", () => {
    assert.equal(
      canManageNote(note({ author_id: "cam-1" }), { id: "admin-1", role: "admin" }),
      true,
    );
  });
  ...
  it("refuses a viewer even for a note with no author left", () => {
    assert.equal(
      canManageNote(note({ author_id: null }), { id: "viewer-1", role: "viewer" }),
      false,
    );
  });
});
...
describe("buildNoteList", () => {
  it("orders newest first and marks which notes the actor may manage", () => {
    const rows = [
    ...
    assert.equal(list.find((n) => n.id === "old")?.canManage, true);
    assert.equal(list.find((n) => n.id === "new")?.canManage, false);
  });
});
