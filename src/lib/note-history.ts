/**
 * F071/F072/F073 — notes-section logic behind the client detail page, kept out
...
/** A note as the page renders it, with whether the viewer may edit it (F073 AC1). */
export type NoteListItem = DisplayNote & { canEdit: boolean };
...
/**
 * Whether `actor` may edit `note` (F073 AC1): the note's own author, or an
 * admin — matching `notes_update_own`'s `with check` exactly
 * (20260804180000_create_org_children.sql), so the button this drives is never
 * shown for an edit the API route would refuse anyway.
 */
