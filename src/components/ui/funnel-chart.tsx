...
/** An SVG `<pattern>` of repeating lines, for use inside a chart's `<defs>`. */
...
export interface FunnelChartProps {
  ...
  showValues?: boolean;
  showLabels?: boolean;
  /** Hover index, when the caller wants to drive it. Omit to keep it internal. */
  ...
  /**
   * 0–1. How much of each segment holds flat at its own value before tapering
   * into the next. Above 0 the percentage pill and the grid line move off the
   * segment boundary and onto the middle of that flat run, so the line marks the
   * point where the funnel really is that stage's size.
   */
  hold?: number;
...
/**
 * `hold` is the fraction of a segment that stays flat at its own value before
 * the taper into the next one begins. It exists so a stage can be *read*: with
 * `hold` at 0 the funnel is at a given stage's width only at the segment's very
 * edge, so a marker showing that stage's share has nowhere honest to sit. With a
 * hold there is a run of chart where the width genuinely is that stage's value,
 * and the marker line and its percentage pill go through the middle of it.
 */
function hSegmentPath(
  ...
  const holdW = segW * hold;

  if (straight) {
    return `M 0 ${my - h0} L ${holdW} ${my - h0} L ${segW} ${my - h1} L ${segW} ${my + h1} L ${holdW} ${my + h0} L 0 ${my + h0} Z`;
  }

  const taperW = segW - holdW;
  const cx = taperW * 0.55;
  const top = `M 0 ${my - h0} L ${holdW} ${my - h0} C ${holdW + cx} ${my - h0}, ${segW - cx} ${my - h1}, ${segW} ${my - h1}`;
  const bot = `L ${segW} ${my + h1} C ${segW - cx} ${my + h1}, ${holdW + cx} ${my + h0}, ${holdW} ${my + h0} L 0 ${my + h0}`;
  return `${top} ${bot} Z`;
}
...
function vSegmentPath(
  ...
  const holdH = segH * hold;

  if (straight) {
    return `M ${mx - w0} 0 L ${mx - w0} ${holdH} L ${mx - w1} ${segH} L ${mx + w1} ${segH} L ${mx + w0} ${holdH} L ${mx + w0} 0 Z`;
  }

  const taperH = segH - holdH;
  const cy = taperH * 0.55;
  const left = `M ${mx - w0} 0 L ${mx - w0} ${holdH} C ${mx - w0} ${holdH + cy}, ${mx - w1} ${segH - cy}, ${mx - w1} ${segH}`;
  const right = `L ${mx + w1} ${segH} C ${mx + w1} ${segH - cy}, ${mx + w0} ${holdH + cy}, ${mx + w0} ${holdH} L ${mx + w0} 0`;
  return `${left} ${right} Z`;
}
...
function HSegment({
  ...
  const rings = Array.from({ length: layers }, (_, l) => {
    ...
    return {
      d: hSegmentPath(normStart, normEnd, segW, fullH, scale, straight, hold),
      opacity,
    };
  });
...
function VSegment({
  ...
  const rings = Array.from({ length: layers }, (_, l) => {
    ...
    const opacity = 0.18 + (l / (layers - 1 || 1)) * 0.65;
    return {
      d: vSegmentPath(normStart, normEnd, segH, fullW, scale, straight, hold),
      opacity,
    };
  });
...
function SegmentLabel({
  ...
  /**
   * 0–1 along the segment where the percentage pill sits. Undefined centres it,
   * which is right when the segment tapers the whole way across; a `hold` puts
   * it over the flat run instead, where the width matches the number on it.
   */
  pctOffset?: number;
}) {
        ...
        {isHorizontal ? (
          <>
            ...
            <div
              className={cn(
                ...
                // `w-full` because the parent is `items-center`, which would
                // otherwise shrink this row to the pill's own width and make the
                // offset a percentage of the pill instead of the segment.
                pctOffset === undefined ? "justify-center" : "relative w-full",
              )}
            >
              {pctOffset === undefined ? (
                pctEl
          ...
          <>
            ...
            <div
              className={cn(
                "flex flex-1 items-center",
                pctOffset === undefined ? "justify-center" : "relative h-full",
              )}
            >
              {pctOffset === undefined ? (
                pctEl
...
export function FunnelChart({
  ...
  /**
   * Where stage `i`'s marker sits, as a 0–1 fraction of its own segment.
   *
   * Every stage after the first sits at 0 — its segment's leading edge, which is
   * exactly where the taper out of the previous stage finishes arriving. That is
   * the only place the marker can be if the shape is to *touch* it: a stage that
   * has fallen to nothing tapers to a point, and the point has to land in the pill
   * reading 0% rather than short of it.
   *
   * The first stage has no taper arriving at it, so it would be pinned to the
   * chart's left edge and cut in half. It sits in the middle of its own flat run
   * instead, which is the same width the whole way across.
   */
  const markerFraction = (i: number) => (i === 0 ? hold / 2 : 0);
        ...
        <>
          ...
          {gridEnabled && showGridLines && (
            <svg
              ...
              viewBox={`0 0 ${W} ${H}`}
            >
              {/* With a hold, there is one line per stage and it runs through the
                  middle of that stage's flat run — the same place its pill sits.
                  Without one, the only meaningful line is the seam between two
                  segments, so there are n-1 of them. */}
              {Array.from({ length: hold > 0 ? n : n - 1 }, (_, i) => {
                const idx = hold > 0 ? i : i + 1;
                if (horiz) {
                  const x =
                    hold > 0
                      ? (segW + gap) * i + segW * markerFraction(i)
                      : segW * idx + gap * i + gap / 2;
                  return (
                    <line
                      key={`grid-${idx}`}
                ...
                const y =
                  hold > 0
                    ? (segH + gap) * i + segH * markerFraction(i)
                    : segH * idx + gap * i + gap / 2;
                return (
                  <line
                    key={`grid-${idx}`}
                    stroke={gridLineColor}
          ...
          {data.map((stage, i) => {
            ...
            return (
              ...
              >
                <SegmentLabel
                  align={labelAlign}
                  pctOffset={hold > 0 ? markerFraction(i) : undefined}
