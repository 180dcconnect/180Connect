...
export async function POST(
  ...
  // author_id/created_at are set here rather than trusted from the client
  // (F072 AC2) — RLS's own with check (author_id = auth.uid()) would refuse a
  // spoofed value anyway, but setting it explicitly is what makes that
  // guarantee visible in this file rather than only in the migration.
