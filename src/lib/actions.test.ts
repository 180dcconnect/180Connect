...
describe("isActionOverdue", () => {
  ...
  it("is overdue when due date is in the past", () => {
    assert.equal(isActionOverdue("2026-08-31", now), true);
  });

  it("is not overdue when due in the future", () => {
    assert.equal(isActionOverdue("2026-09-02", now), false);
  });
});
...
describe("formatMyActions (F168)", () => {
  ...
  it("flags an overdue action (F172 AC1)", () => {
  ...
  it("sorts overdue and soon-due actions before undated ones", () => {
    const actions = formatMyActions(
      [
        ...
        row({ id: "soon", due_date: "2026-09-02" }),
      ],
      ACTOR_ID,
      now,
    );
...
describe("groupMyActionsByDueDate (F172 AC1/AC2)", () => {
  const now = new Date("2026-09-01T09:00:00Z");
  ...
  it("puts an action due today in upcoming, not overdue", () => {
    const actions = formatMyActions([row({ due_date: "2026-09-01" })], ACTOR_ID, now);
    const groups = groupMyActionsByDueDate(actions);
