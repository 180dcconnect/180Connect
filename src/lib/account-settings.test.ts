...
import { MAX_FULL_NAME_LENGTH, parseAccountSettings } from "./account-settings.ts";

/**
 * Invisible characters are written as escapes rather than pasted literally so
 * that the intent of each case survives being read, diffed and rewritten.
 */
...
test("ignores unknown extra keys (F178: frequency moved to notification-preferences)", () => {
  // The profile form no longer submits notification_frequency, and the parse
  // refuses to care about it — the setting is owned by /settings/notifications.
  const result = parseAccountSettings({
    fullName: "Bashir",
    notificationFrequency: "hourly",
  } as { fullName: string });
  assert.equal(result.ok, true);
  assert.equal(result.ok && result.value.fullName, "Bashir");
});
