...
describe("buildNoteEntries", () => {
  ...
  it("produces a second entry at updated_at for an edited note (AC1)", () => {
...
describe("buildTimeline", () => {
  it("merges every source into one feed, newest first (AC1)", () => {
  ...
  it("drops an audit row whose action this timeline does not render", () => {
    const timeline = buildTimeline(
      {
        notes: [],
        outreachMessages: [],
  ...
  it("interleaves a note's edited entry among later events, not only under its original date (AC1)", () => {
    const timeline = buildTimeline(
      {
        notes: [
          noteRow({
